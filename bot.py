...
def get_default_config():
    return {
        "target_channel": "",
        "influencer_discount_percent": 15,
        "influencer_discount_max": 50,
        "card_discount_percent": 25,
        "card_discount_max": 75
    }
...
def get_product_info(url):
    ...
    try:
        ...
        time.sleep(0.5)
        response = session.get(url, allow_redirects=True, timeout=20, verify=False)
        ...
        price_patterns = [
            soup.find("span", {"class": re.compile("a-price-whole")}),
            soup.find("span", {"class": re.compile("a-price")}),
        ...
        for price_elem in price_patterns:
            if price_elem:
                try:
                    ...
                    price_text = price_text.replace(',', '').replace('ر.س', '').replace('SAR', '').strip()
        ...
        if not price:
            for span in soup.find_all("span"):
                try:
                    ...
                    if re.search(r'\d+\.\d+', text) and ('ر.س' in text or 'SAR' in text or 'riyal' in text.lower()):
        ...
        img_patterns = [
            ...
            soup.find("img", {"class": re.compile("a-dynamic-image")}),
        ]
            ...
            if img_tag:
                ...
                if dyn_img:
                    try:
                        ...
                        if img_dict:
                            image = list(img_dict.keys())[-1]
                            break
                    except:
                        pass
                
                src = img_tag.get("src")
                if src and ('amazon' in src or 'ssl-images' in src):
                    image = src
                    break
...
def calculate_final_price(original_price, config):
    """حساب السعر النهائي مع الخصومات بالحد الأقصى"""
    if not original_price:
        return None, None, None
    
    # خصم المؤثرين: 15% بحد أقصى 50 ريال
    influencer_discount = min(
        original_price * (config["influencer_discount_percent"] / 100),
        config["influencer_discount_max"]
    )
    
    # خصم بطاقة مدى: 25% بحد أقصى 75 ريال
    card_discount = min(
        original_price * (config["card_discount_percent"] / 100),
        config["card_discount_max"]
    )
    
    # السعر النهائي
    final_price = original_price - influencer_discount - card_discount
    
    return final_price, influencer_discount, card_discount

def format_price(price):
    if not price:
        return "---"
    return f"{price:.0f}" if price == int(price) else f"{price:.1f}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "فقط أرسل رابط المنتج وسيقوم البوت بسحب الصورة والاسم وحساب السعر بعد الخصومات تلقائياً.\n\n"
        ...
        "• سيقوم البوت بدمج صور المنتجات في صورة واحدة!\n\n"
        "3️⃣ <b>إعدادات القناة:</b>\n"
        "استخدم /set_channel @اسم_القناة لتحديد القناة التي سيتم النشر فيها.\n\n"
        "📢 <b>القناة الحالية:</b> <code>" + (config.get("target_channel") or "لم تحدد بعد") + "</code>\n\n"
        "💰 <b>الخصومات المفعلة:</b>\n"
        f"• كود المؤثرين: {config['influencer_discount_percent']}% (حد أقصى {config['influencer_discount_max']} ريال)\n"
        f"• خصم بطاقة مدى: {config['card_discount_percent']}% (حد أقصى {config['card_discount_max']} ريال)"
    )
...
async def bundle_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    image_urls = []
    for item in items:
        final_price, inf_disc, card_disc = calculate_final_price(item['original_price'], config)
        price_text = format_price(final_price)
        msg += f"{item['name']}\n<b>بسعر {price_text} ريال بس</b> 🔥\n\nالرابط:\n{item['url']}\n\n\n"
        if item['image']: 
            image_urls.append(item['image'])
    
    msg += f"✅ استخدام كود خصم المؤثرين {config['influencer_discount_percent']}% (حد أقصى {config['influencer_discount_max']} ريال)\n\n"
    msg += f"✅ فعل خصم بطاقة مدى بنك الرياض (مجانية) {config['card_discount_percent']}% (حد أقصى {config['card_discount_max']} ريال)\n\n"
    msg += "الحد الأدنى للطلب 100 ريال عشان يتفعل الخصم"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    config = load_config()
    final_price, inf_disc, card_disc = calculate_final_price(info['original_price'], config)
    price_text = format_price(final_price)
    
    msg = (
        f"<b>{info['name']}</b>\n\n"
        f"<b>بسعر {price_text} ريال تقريبًا بس</b> 🔥\n\n"
        f"الرابط: {url}\n\n"
        f"✅ استخدام كود خصم المؤثرين {config['influencer_discount_percent']}%\n\n"
        f"✅ فعل خصم بطاقة مدى بنك الرياض {config['card_discount_percent']}%\n\n"
