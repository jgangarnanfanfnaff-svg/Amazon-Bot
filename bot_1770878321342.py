...
# إعداد Flask لعمل Web Server بسيط لإبقاء البوت حياً
app = Flask('')

# تعطيل السجلات الطويلة من Werkzeug
import logging
...
# إعداد السجلات
# إعداد السجلات بدون طباعة السجلات الطويلة
...
def get_product_info(url):
    ...
    try:
        ...
        # لا نقوم بتغيير الرابط للحفاظ على كود العمولة، فقط نستخدمه لجلب البيانات
        ...
        if price_span:
            try: price = float(price_span.get_text().replace(',', '').replace('.', '').strip())
            except: pass
        if not price:
            price_meta = soup.find("span", {"class": "a-offscreen"})
            if price_meta:
                try: price = float(re.sub(r'[^\d.]', '', price_meta.get_text()))
                except: pass
        ...
        if img_tag:
            ...
            if dyn_img:
                try: image = list(json.loads(dyn_img).keys())[-1]
...
def create_collage(image_urls):
    images = []
    # نكتفي بأول 4 صور لتجنب استهلاك الذاكرة والتهنيق
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'publish':
        ...
        img = context.user_data.get('pending_img') # للمجمع (BytesIO)
        img_url = context.user_data.get('pending_img_url') # للفردي (URL)
